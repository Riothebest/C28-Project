...
function draw() {
  ...
  for (var i = 0; i < playerArrows.length; i++) {
    if (playerArrows[i] !== undefined) {
      ...
      d1 = dist(playerArrows[i].body.position.x,playerArrows[i].body.position.y, 
                board1.body.position.x,board1.body.position.y);
      if(d1<=100)
      {
  ...
  text("Score: "+score,width-700,180)

  if (numberOfArrows == 0 && score <60) {
    youLose();
  }
  if (score >=60) {
    youWin();
...
function youLose()
{
  swal(
    {
        title: "Game Over !",
        text: "You couldn't get over 60 points!",
        imageSize: "250 x 250",
